...
******************************************************/
   ...
   FILE *OpenFile (const char * const fn_p,
...
int GenMatrixFile (const char * const matrix_fn_p,
...
unsigned int *AllocateMatrixSpace (
   const unsigned int M_WIDTH, // matrix M width/N's height
   const unsigned int M_HEIGHT, // matrix M and P's height
   const unsigned int N_WIDTH) { // matrix N and P's width
   unsigned int M_SIZE;
   unsigned int * matrix = NULL;
   M_SIZE = M_HEIGHT*M_WIDTH;
   if (M_SIZE<M_WIDTH*N_WIDTH) M_SIZE=M_WIDTH*N_WIDTH;
   if (M_SIZE<M_HEIGHT*N_WIDTH) M_SIZE=M_HEIGHT*N_WIDTH;
   matrix = (unsigned int *) malloc(M_SIZE * sizeof(unsigned int));
   return (matrix);
}

void ReadMatrixFile (unsigned int * matrix,
...
int WriteMatrixFile (const char * const matrix_fn_p,
...
void CompareMatrixFile (const char * const matrix_fn_p1,
   ...
   int check_ok = 1;
   unsigned int *m1=(unsigned int *)malloc(sizeof(unsigned int)*M_WIDTH*M_HEIGHT); 
   ReadMatrixFile (m1,matrix_fn_p1,M_WIDTH, M_HEIGHT, if_silent);
   unsigned int *m2=(unsigned int *) malloc(sizeof(unsigned int)*M_WIDTH*M_HEIGHT);
   ReadMatrixFile (m2,matrix_fn_p2,M_WIDTH, M_HEIGHT, if_silent);
